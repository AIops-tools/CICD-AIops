...
throwaway home (``CICD_AIOPS_HOME``) and assert that the rows compliance
...
from cicd_aiops.governance import PolicyDenied, governed_tool
...
def gov_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("CICD_AUDIT_APPROVED_BY", raising=False)
    monkeypatch.delenv("CICD_POLICY_DISABLED", raising=False)
    _reset_singletons()
    yield tmp_path
    _reset_singletons()
...
def test_medium_write_persists_audit_and_undo_rows(gov_home):
    result = _rename_widget(name="w1", target="gl1")
...
def test_high_risk_denied_without_approver_and_denial_is_audited(gov_home):
    ...
    with pytest.raises(PolicyDenied, match="requires 'dual' approval"):
        _drop_widget(name="w2", target="gl1")
...
def test_high_risk_allowed_with_named_approver(gov_home, monkeypatch):
    monkeypatch.setenv("CICD_AUDIT_APPROVED_BY", "cicd-ops-alice")
    result = _drop_widget(name="w3", target="gl1")
    assert result["status"] == "dropped"

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert len(audit) == 1
    assert audit[0]["status"] == "ok"
    assert audit[0]["approved_by"] == "cicd-ops-alice"
...
def test_operator_rules_file_restores_tier_none_for_high_risk(gov_home):
    ...
    _reset_singletons()
    result = _drop_widget(name="w4", target="gl1")
    assert result["status"] == "dropped"


@pytest.mark.unit
def test_real_write_tool_persists_priorstate_undo(gov_home, monkeypatch):
    """End-to-end through a REAL product write tool: pause_runner must capture
    the runner's prior paused state before flipping and persist the
    resume-undo on disk."""
    ...
    result = gov.pause_runner(runner="7")
    assert result["priorState"] == {"paused": False}
    assert result.get("_undo_id")

    undo = _rows(gov_home / "undo.db", "undo_log")
    assert len(undo) == 1
    assert undo[0]["undo_tool"] == "resume_runner"
    assert json.loads(undo[0]["undo_params"]) == {"runner": "7"}

    audit = _rows(gov_home / "audit.db", "audit_log")
    assert [r["tool"] for r in audit] == ["pause_runner"]
