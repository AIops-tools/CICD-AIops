"""Tests for the ``cicd-aiops init`` onboarding wizard.

The wizard is driven end-to-end through Typer's CliRunner with every path
(config.yaml, secrets.enc) isolated under tmp_path. The master
password comes from CICD_AIOPS_MASTER_PASSWORD (the non-interactive path)
and the hidden access-token prompt is patched at the getpass boundary.
"""

from __future__ import annotations

import getpass as getpass_mod

import pytest
import yaml
from typer.testing import CliRunner

import cicd_aiops.cli.init as init_mod
import cicd_aiops.config as config_mod
import cicd_aiops.doctor as doctor_mod
import cicd_aiops.secretstore as ss

MASTER_PW = "init-master-pw"
API_TOKEN = "glpat-secret-0123"

# Wizard answers: name, accept platform default (gitlab), base URL, accept
# TLS-verify default (True), no second target, decline the trailing doctor
# run. The access token itself comes via getpass.
WIZARD_INPUT = "gl1\n\nhttps://git.example.com\n\nn\nn\n"


@pytest.fixture
def init_home(tmp_path, monkeypatch):
    """Isolate config + secret store + governance home under tmp_path."""
    config_file = tmp_path / "config.yaml"
    secrets_file = tmp_path / "secrets.enc"
    monkeypatch.setenv("CICD_AIOPS_HOME", str(tmp_path))
    monkeypatch.setenv("CICD_AIOPS_MASTER_PASSWORD", MASTER_PW)
    monkeypatch.setattr(init_mod, "CONFIG_DIR", tmp_path)
    monkeypatch.setattr(init_mod, "CONFIG_FILE", config_file)
    monkeypatch.setattr(config_mod, "CONFIG_DIR", tmp_path)
    monkeypatch.setattr(config_mod, "CONFIG_FILE", config_file)
    monkeypatch.setattr(ss, "CONFIG_DIR", tmp_path)
    monkeypatch.setattr(ss, "SECRETS_FILE", secrets_file)
    monkeypatch.setattr(ss, "LEGACY_ENV_FILE", tmp_path / ".env")
    monkeypatch.setattr(ss, "_cached", None)
    # The hidden token prompt bypasses CliRunner stdin.
    monkeypatch.setattr(getpass_mod, "getpass", lambda prompt="": API_TOKEN)
    return tmp_path


def _run_init(input_text: str = WIZARD_INPUT):
    from cicd_aiops.cli import app

    return CliRunner().invoke(app, ["init"], input=input_text)


@pytest.mark.unit
def test_init_writes_config_with_entered_values(init_home):
    result = _run_init()
    assert result.exit_code == 0, result.output
    raw = yaml.safe_load((init_home / "config.yaml").read_text("utf-8"))
    assert raw["targets"] == [
        {
            "name": "gl1",
            "platform": "gitlab",
            "base_url": "https://git.example.com",
            "verify_ssl": True,  # accepted TLS confirm default=True must land
        }
    ]


@pytest.mark.unit
def test_init_tls_confirm_can_be_declined_for_lab_certs(init_home):
    result = _run_init("gl1\n\nhttps://git.example.com\nn\nn\nn\n")
    assert result.exit_code == 0, result.output
    raw = yaml.safe_load((init_home / "config.yaml").read_text("utf-8"))
    assert raw["targets"][0]["verify_ssl"] is False


@pytest.mark.unit
def test_init_gitea_branch(init_home):
    result = _run_init("gt1\ngitea\nhttps://gitea.example.com\n\nn\nn\n")
    assert result.exit_code == 0, result.output
    raw = yaml.safe_load((init_home / "config.yaml").read_text("utf-8"))
    assert raw["targets"][0]["platform"] == "gitea"
    assert ss.SecretStore.unlock(MASTER_PW).get("gt1") == API_TOKEN


@pytest.mark.unit
def test_init_rejects_unknown_platform_then_reprompts(init_home):
    result = _run_init(
        "gl1\njenkins-x\ngl1\n\nhttps://git.example.com\n\nn\nn\n"
    )
    assert result.exit_code == 0, result.output
    assert "Platform must be 'gitlab' or 'gitea'." in result.output
    raw = yaml.safe_load((init_home / "config.yaml").read_text("utf-8"))
    assert [t["name"] for t in raw["targets"]] == ["gl1"]


@pytest.mark.unit
def test_init_stores_token_encrypted_not_in_config(init_home):
    result = _run_init()
    assert result.exit_code == 0, result.output
    # The token is readable back through the secret store API...
    assert ss.SecretStore.unlock(MASTER_PW).get("gl1") == API_TOKEN
    # ...and never lands in plaintext in config.yaml or secrets.enc.
    assert API_TOKEN not in (init_home / "config.yaml").read_text("utf-8")
    assert API_TOKEN not in (init_home / "secrets.enc").read_text("utf-8")


@pytest.mark.unit
def test_init_writes_no_policy_rules(init_home):
    """The skill no longer authorizes, so init seeds no rules.yaml — a fresh
    install delivers full functionality and leaves permission to the account."""
    result = _run_init()
    assert result.exit_code == 0, result.output
    assert not (init_home / "rules.yaml").exists()


@pytest.mark.unit
def test_init_accepting_doctor_confirm_runs_doctor(init_home, monkeypatch):
    calls: list[bool] = []
    monkeypatch.setattr(doctor_mod, "run_doctor", lambda: calls.append(True) or 0)
    # Empty last answer accepts the confirm's default=True.
    result = _run_init("gl1\n\nhttps://git.example.com\n\nn\n\n")
    assert result.exit_code == 0, result.output
    assert calls == [True]


@pytest.mark.unit
def test_init_overwrite_existing_target(init_home):
    result = _run_init()
    assert result.exit_code == 0, result.output
    # Same name again: confirm overwrite, new URL, accept defaults.
    result = _run_init("gl1\ny\n\nhttps://git-new.example.com\n\nn\nn\n")
    assert result.exit_code == 0, result.output
    raw = yaml.safe_load((init_home / "config.yaml").read_text("utf-8"))
    assert [t["base_url"] for t in raw["targets"]] == ["https://git-new.example.com"]
