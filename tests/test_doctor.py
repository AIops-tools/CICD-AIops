...
ever touches a real GitLab/Gitea server or ``~/.cicd-aiops``.
"""
...
import cicd_aiops.config as config_mod
import cicd_aiops.connection as connection_mod
import cicd_aiops.doctor as doctor_mod
...
def doctor_home(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("CICD_GL1_SECRET", raising=False)
    return tmp_path
...
def _seed_secret(monkeypatch, name: str = "gl1", value: str = "glpat-token-1") -> None:
    monkeypatch.setenv("CICD_AIOPS_MASTER_PASSWORD", MASTER_PW)
...
class _FakeConn:
    ...
    def get(self, path):
        if path == self.platform.path("version"):
            return {"version": "17.2.1"}
        assert path == self.platform.path("current_user")
        return {"username": "ops-bot"}
...
class _UnreachableManager:
    ...
    def connect(self, name):
        raise ConnectionError("HTTPS connection to 'git.example.com' refused")


class _BadTokenConn(_FakeConn):
    """Version works (anonymous-ish) but the token-scope probe 401s."""

    def get(self, path):
        if path == self.platform.path("version"):
            return {"version": "17.2.1"}
        raise PermissionError("401: token lacks read_api scope")


class _BadTokenManager(_HealthyManager):
    def connect(self, name):
        return _BadTokenConn(self._config.get_target(name))
...
def test_doctor_all_healthy_exit_zero(doctor_home, monkeypatch, capsys):
    ...
    assert "Token present for 'gl1' (gitlab)" in out
    assert "Connected to 'gl1' (gitlab https://git.example.com)" in out
    assert "server version 17.2.1" in out
    assert "Token scope probe OK for 'gl1' — authenticated as 'ops-bot'" in out
...
def test_doctor_unreachable_target_exit_one(doctor_home, monkeypatch, capsys):
    ...
    assert run_doctor() == 1
    out = capsys.readouterr().out
    assert "Connect to 'gl1' failed" in out
    assert "refused" in out


@pytest.mark.unit
def test_doctor_bad_token_scope_probe_exit_one(doctor_home, monkeypatch, capsys):
    """The version endpoint answering but the token probe failing must be
    reported as a problem — a reachable server with a dud token is unhealthy."""
    _write_config(doctor_home, [_TARGET])
    _seed_secret(monkeypatch)
    monkeypatch.setattr(connection_mod, "ConnectionManager", _BadTokenManager)
    assert run_doctor() == 1
    out = capsys.readouterr().out
    assert "server version 17.2.1" in out
    assert "Token scope probe failed for 'gl1'" in out
...
def test_doctor_no_secret_store_and_no_secret(doctor_home, capsys):
    ...
    assert "No token for target 'gl1'" in out


@pytest.mark.unit
def test_doctor_legacy_env_file_warns_migrate(doctor_home, monkeypatch, capsys):
    _write_config(doctor_home, [_TARGET])
    (doctor_home / ".env").write_text("CICD_GL1_SECRET=legacy\n", "utf-8")
    monkeypatch.setenv("CICD_GL1_SECRET", "legacy")
    assert run_doctor(skip_auth=True) == 0
    out = capsys.readouterr().out
