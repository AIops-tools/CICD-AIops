...
from cicd_aiops.governance import READ_ONLY_ENV, is_read_only
...
def test_apply_read_only_unregisters_write_tools(monkeypatch):
    ...
    try:
        ...
        assert "delete_artifacts" in dropped and "pause_runner" in dropped
        assert "cancel_pipeline" in dropped and "update_branch_protection" in dropped
        assert "list_pipelines" not in dropped and "job_trace_tail" not in dropped
        ...
        assert "list_pipelines" in remaining, "reads must still be exposed"
        assert "pipeline_failure_rca" in remaining, "the RCAs are reads"
    finally:
        registry.clear()
...
@pytest.mark.unit
def test_governed_cicd_write_tool_is_refused_in_read_only(read_only):
    """The real MCP write path is covered, not just a synthetic tool."""
    from mcp_server.tools import writes as gov

    with pytest.raises(PolicyDenied):
        gov.pause_runner(runner="7")
