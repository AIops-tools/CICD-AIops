...
**The behaviour** — ``emit_governed()`` aborts non-zero on an error payload and on an
...
from cicd_aiops.cli._common import emit_governed

CLI_DIR = pathlib.Path(__file__).resolve().parents[1] / "cicd_aiops" / "cli"
...
CHECKERS = ['dry_run_preview', 'emit_governed']


@pytest.mark.unit
def test_checked_aborts_on_error_payload():
    with pytest.raises(typer.Exit) as exc:
        emit_governed({"error": "refused: that would lock this tool out"})
...
def test_checked_passes_a_successful_payload_through():
    ...
    assert emit_governed(payload) == payload
