"""``cicd-aiops secret`` — manage the encrypted credential store.

Secrets (CI/CD access tokens) are stored in ``~/.cicd-aiops/secrets.enc``
...
from cicd_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
...
def secret_list() -> None:
    ...
    if not names:
        console.print(
            "[yellow]No secrets stored yet. Add one: cicd-aiops secret set <name>[/]"
        )
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update CICD_AIOPS_MASTER_PASSWORD.[/]")
