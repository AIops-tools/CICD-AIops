"""Unified audit logging engine — all CI/CD skills write to a single SQLite database.

Replaces 7 per-skill JSON Lines audit loggers with one shared ``~/.cicd-aiops/audit.db``.
...
from cicd_aiops.governance.paths import ops_path

_log = logging.getLogger("cicd-aiops.audit")

# Back-compat override hook. When None (default) the path resolves via
# ops_path("audit.db") (honoring CICD_AIOPS_HOME). Downstream code/tests may set this
# to a fixed Path to redirect the default DB — kept so the CICD_AIOPS_HOME refactor
