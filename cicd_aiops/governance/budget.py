...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``CICD_RUNAWAY_MAX`` times within ``CICD_RUNAWAY_WINDOW_SEC`` trips a
...
from cicd_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing CI/CD-specific; the decorator wraps it into a
