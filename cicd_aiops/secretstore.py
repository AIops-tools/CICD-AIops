"""Encrypted secret storage for cicd-aiops.

Secrets (CI/CD access tokens) are NEVER written to disk in plaintext. They
live in ``~/.cicd-aiops/secrets.enc`` encrypted with Fernet (AES-128-CBC +
...
The master password is resolved (in order) from:
  ...
  1. the env var ``CICD_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
APP_NAME = "cicd-aiops"
CONFIG_DIR = ops_home()
MASTER_PASSWORD_ENV = "CICD_AIOPS_MASTER_PASSWORD"  # nosec B105 — env-var name
# ──────────────────────────────────────────────────────────────────────────────
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``CICD_<TARGET>_SECRET``; ``prefix='CICD_'`` and
